...
const ProductFlow = {
    URL: "/v1/tenants/KOREA/product/products/",
    METHOD: "POST"
};
...
productBtn.addEventListener("click", () => {
    progressBarDisplay(3, "Y");
    const targetPattern = getConstant(productBtn.dataset['code']);
    const productCode = document.querySelector("#productCodeDiv").innerText.replaceAll("/^[A-Za-z0-9]+$/","");

    console.log(`url : ${targetUrl}${targetPattern.URL}${productCode} \nmethod : ${targetPattern.METHOD}`);

    //fetch api
    fetch(targetUrl + targetPattern.URL + productCode, {
        method: targetPattern.METHOD,
        ...
        return new Promise((resolve) => response.json().then((responseData) => resolve({
        ...
        switch(status) {
            case 200:
                productCallback(productCode, responseData);
                break;
            case 400:
                progressBarDisplay(3, "N");
...
const productCallback = (productCode, responseData) => {
    ...
    // createElement(elementList, "td", responseData.illustration.productCode);
    // createElement(elementList, "td", responseData.illustration.productName);
    // createElement(elementList, "td", responseData.illustration.insurances[0].insuranceCode);
    // createElement(elementList, "td", responseData.illustration.beforeDiscountPremiumSum.amount);
    // createElement(elementList, "td", responseData.illustration.afterInsuredBirthPremiumSum.amount);
...
premiumBtn.addEventListener("click", () => {
    ...
    console.log(`url : ${targetUrl}${targetPattern.URL} \nmethod : ${targetPattern.METHOD}`);

    //fetch api
    fetch(targetUrl + targetPattern.URL, {
        method: targetPattern.METHOD,
        body: targetPattern.METHOD === "GET" ? null : illustration,
        headers: {
            "Content-Type": "application/json"
...
const surrenderCallback = (illustration, responseData) => {
    ...
    responseData.illustration.surrenderList.forEach(surrender => {
        elementList = new Array();
...
const benefitCallback = (illustration, responseData) => {
    if(responseData.illustration[0].benefitContents == null) return;
    ...
    responseData.illustration[0].benefitContents.forEach(benefitContent => {
        elementList = new Array();
    ...
    responseData.illustration[0].benefitDisbursementNotes.forEach(benefitDisbursementNote => {
        elementList = new Array();
...
const el2Obj = (data) => {
    ...
    if(type === "main") {
        ...
        if(undefined !== name && "" !== name) rtData[name] = subData;
        else {
            rtData = {};
            rtData = {
                ...subData
...
const obj2El = (data, objName, isArray) => {
    ...
    if("string" === objTypeof || "number" === objTypeof || "boolean" === objTypeof) {
        ...
        if(objName === "customerTypeCode") {
            rtElTxt = '<div id="' + id + '" data-type="select" data-name="${dataName}" contenteditable="false" data-code="${data}" data-valuetype="' + objTypeof + '">' + '<span class="addCodeBtn" onClick=\'makeModal("' + id + '",CustomerTypeCode)\'>' + CustomerTypeCode[data] + '</span></div>';
        }
        if(objName === "insuranceTypeCode") {
            rtElTxt = '<div id="' + id + '" data-type="select" data-name="${dataName}" contenteditable="false" data-code="${data}" data-valuetype="' + objTypeof + '">' + '<span class="addCodeBtn" onClick=\'makeModal("' + id + '", InsuranceTypeCode)\'>' + InsuranceTypeCode[data] + '</span></div>';
        }
        if(objName === "billingFrequencyCode") {
            rtElTxt = '<div id="' + id + '" data-type="select" data-name="${dataName}" contenteditable="false" data-code="${data}" data-valuetype="' + objTypeof + '">' + '<span class="addCodeBtn" onClick=\'makeModal("' + id + '", BillingFrequencyCode)\'>' + BillingFrequencyCode[data] + '</span></div>';
        }
        if(objName === "insuredPeriodTypeCode") {
            rtElTxt = '<div id="' + id + '" data-type="select" data-name="${dataName}" contenteditable="false" data-code="${data}" data-valuetype="' + objTypeof + '">' + '<span class="addCodeBtn" onClick=\'makeModal("' + id + '", PeriodTypeCode)\'>' + PeriodTypeCode[data] + '</span></div>';
        }
